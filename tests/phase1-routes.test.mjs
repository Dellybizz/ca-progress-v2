...
test("mobile navigation is independently designed with real routes", () => { const mobile = read("components/shell/mobile-navigation.tsx"); assert.match(mobile, /BottomSheet/); assert.match(mobile, /\/planner/); assert.match(mobile, /\/progress/); assert.match(mobile, /\/study/); assert.equal(mobile.includes('href="#"'), false); });
