...
test("Product Phase 1 destinations consume and validate Chapter Hub academic context", () => {
  ...
  assert.match(studyTimer, /model\.subjects\.find\(\(subject\) => subject\.id === requestedSubjectId\)/);
  assert.match(studyTimer, /initialSubject\.chapters\.some\(\(chapter\) => chapter\.id === requestedChapterId\)/);
...
test("Product Phase 1 Chapter Hub private data is cross-user scoped and academic lookup is cross-level scoped", () => {
  ...
  assert.match(service, /FROM study_sessions s[\s\S]{0,500}WHERE s\.user_id=\?1 AND s\.chapter_id=\?2/);
