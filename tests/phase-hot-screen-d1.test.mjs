...
test("hot screens have typed direct-D1 repository coverage", () => {
  ...
  for (const name of ["getHotPlannerRows", "getHotCalendarRows", "getHotActivityRows", "getHotProgressRows", "getHotDashboardProgress", "getHotCommunityChannel", "getHotCommunityMessages"]) assert.match(hot, new RegExp(`export async function ${name}`));
...
test("hot-query D1 migration remains idempotent and deployment validates the retained migration chain", () => {
  ...
  assert.match(validator, /"migrations",\s*"apply"/);
