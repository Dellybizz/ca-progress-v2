...
test("Phase 10 remains implemented after the V2 staging label advances to Phase 11",()=>{assert.match(read("wrangler.jsonc"),/NEXT_PUBLIC_APP_VERSION\": \"phase-11\"/);assert.match(read(".env.example"),/NEXT_PUBLIC_APP_VERSION=phase-11/);assert.match(read("lib/env.ts"),/\|\| \"phase-11\"/);assert.match(read(".github/workflows/deploy-staging.yml"),/NEXT_PUBLIC_APP_VERSION:\s*phase-11/);assert.match(read("app/globals.css"),/phase10\.css/);});
