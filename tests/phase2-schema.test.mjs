...
test("phone OTP support is cleanly retired from the current schema", () => { assert.match(socialCleanup, /drop table if exists public\.auth_otp_rate_limits/i); const types = readFileSync(join(root, "lib/supabase/database.types.ts"), "utf8"); assert.equal(types.includes("auth_otp_rate_limits"), false); });
