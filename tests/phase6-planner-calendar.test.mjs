...
test("calendar is a context-scoped timezone-aware composed view over current source tables",()=>{const service=read("lib/planner/calendar.ts");for(const source of ["tasks","goals","user_calendar_events","exam_attempts","exam_events"]) assert.match(service,new RegExp(`from\\(\"${source}\"\\)`));assert.match(service,/getStudentContext/);assert.match(service,/localMonthKey/);assert.match(service,/context\.timezone/);assert.match(service,/verification_status.*verified/);assert.match(service,/readOnly: true/);});
