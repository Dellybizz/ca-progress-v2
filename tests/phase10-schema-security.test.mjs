...
test("authenticated clients are read-only at table level and writes use guarded RPCs",()=>{const s=sql();assert.match(s,/revoke insert,update,delete,truncate,references,trigger[\s\S]*from authenticated/);assert.match(s,/grant select[\s\S]*to authenticated/);assert.match(s,/phase10_can_write_channel/);assert.match(s,/raise exception 'You cannot write to this channel.'/);assert.match(s,/v_policy\s*=\s*'members'\s+or\s+public\.phase10_is_moderator/);});
test("announcement channels are moderator-write and subject visibility is level/group scoped",()=>{const s=sql();assert.match(s,/\('announcements','Announcements'[\s\S]*'moderators'/);assert.match(s,/v_profile\.ca_level\s*<>\s*v_level_code/);assert.match(s,/v_profile\.group_choice\s*=\s*'both'\s+or\s+v_profile\.group_choice\s*=\s*v_group_code/);});
