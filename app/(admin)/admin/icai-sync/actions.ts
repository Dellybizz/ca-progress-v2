...
export async function recoverIcaiStartupJobAction(formData: FormData) {
  ...
  try {
    const operator = await requireAdminOperator();
    const jobId = cleanText(formData.get("jobId"), 200);
    ...
    if (!current.data || !["queued", "running"].includes(current.data.status)) {
      throw new Error("This synchronization job is no longer active.");
    }
    const anchor = new Date(current.data.started_at ?? current.data.created_at).getTime();
    if (!Number.isFinite(anchor) || Date.now() - anchor < 2 * 60_000) {
      throw new Error("This job is still within its startup window.");
    }
    const now = new Date().toISOString();
    const update = await admin
      ...
      .update({
        status: "failed",
        finished_at: now,
        last_error: "Orphaned ICAI job recovered before a sync run started.",
        updated_at: now,
      })
      .eq("id", jobId)
      .in("status", ["queued", "running"]);
    ...
    await auditControl({
      action: "recover_stale_job",
      reason: `job_id=${jobId}; no sync run started`,
