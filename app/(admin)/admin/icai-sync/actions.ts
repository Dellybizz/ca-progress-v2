...
export async function controlIcaiSyncAction(formData: FormData) {
  ...
  try {
    ...
    if (!runId || !["cancel", "skip", "recover"].includes(intent)) throw new Error("Invalid synchronization control request.");
    const admin = createD1AdminClient();
    ...
    if (intent === "recover") {
      ...
      const update = await admin.from("icai_sync_runtime").update({ ...(intent === "cancel" ? { cancel_requested: true } : { skip_source_requested: true }), control_requested_by: operator.user.id, control_reason: intent === "cancel" ? "admin_cancel" : "admin_skip_source", updated_at: new Date().toISOString() }).eq("run_id", runId);
      if (update.error) throw update.error;
      destination = `/admin/icai-sync?notice=${encodeURIComponent(intent === "cancel" ? "Cancellation requested. The worker will stop at its next checkpoint." : "Skip requested. The worker will preserve existing data and continue with the next source.")}`;
    }
