...
export default async function MembersPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; role?: string }> }) {
  ...
  catch { return <AdminDenied message="Admin, owner or parent-owner access is required to view the member directory."/>; }
  ...
  return <div className="phase12-page">
    <PageHeader preview={false} eyebrow="Operations · Members" title="Members and access" description="Search a server-paginated directory, inspect current subscription resolution and manage privileged roles without loading the full user base into memory."/>
    ...
    {model.rows.length ? <div className="phase12-table-wrap"><table className="phase12-table"><thead><tr><th>Member</th><th>Access</th><th>Current plan</th><th>Joined</th><th>Operations</th></tr></thead><tbody>{model.rows.map((row) => <tr key={row.user_id}><td><strong>{row.display_name || "Member"}</strong><small>{row.email || row.user_id}</small></td><td><span className="phase12-role-pill">{row.role.replace("_"," ")}</span>{row.role !== "student" ? <small>{row.admin_active ? "Admin access active" : "Admin access disabled"}</small> : null}</td><td><strong>{row.plan_name}</strong><small>{row.subscription_ends_at ? `Active until ${new Date(row.subscription_ends_at).toLocaleDateString("en-IN")}` : row.plan_tier === "free" ? "Free fallback" : "No expiry"}</small></td><td>{new Date(row.user_created_at).toLocaleDateString("en-IN")}</td><td>{canManage && row.user_id !== operator.userId ? <MemberRoleControl userId={row.user_id} currentRole={row.role} active={row.admin_active} operatorRole={operator.role}/> : <span className="phase12-muted">{row.user_id === operator.userId ? "Your account" : "View only"}</span>}</td></tr>)}</tbody></table></div> : <EmptyState icon="community" title="No members match" description="Try clearing the filters or searching another name or email."/>}
