...
export default async function PlansAdminPage() {
  ...
  catch { return <AdminDenied message="Admin, owner or parent-owner access is required to inspect plan configuration."/>; }
  const model = await getPlansAdminModel();
  const canEdit = canManagePlatform(operator.role);
  return <div className="phase12-page"><PageHeader preview={false} eyebrow="Operations · Billing" title="Plans and feature access" description="Commercial values and entitlement limits stay server-authoritative. Owner changes are validated in the database and written to the immutable operations audit log."/>
