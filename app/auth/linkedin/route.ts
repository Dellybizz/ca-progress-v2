...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await supabase.auth.signInWithOAuth({ provider: "linkedin_oidc", options: { redirectTo: callback.toString() } });
    if (error || !data.url) throw error ?? new Error("LinkedIn OAuth URL was not returned.");
