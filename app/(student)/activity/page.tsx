...
async function ActivityContent() {
  const model = await getActivityPageModel();
  ...
  return <div className="phase6-page"><PageHeader preview={false} eyebrow="Activity" title="Your study and progress history, generated automatically." description="This timeline is composed from completed study sessions and normalized progress events — no duplicate activity log is maintained." actions={<div className="phase6-header-links"><Link href="/study">Study</Link><Link href="/progress">Progress</Link></div>}/><Card><CardBody>{model.items.length ? <div className="phase6-activity-list">{model.items.map((item) => <Link key={item.id} href={item.href}><span className={`phase6-activity-icon phase6-activity-icon--${item.source}`}><Icon name={item.source === "study" ? "timer" : "chart"}/></span><span><strong>{item.title}</strong><small>{item.description}</small></span><time>{new Date(item.occurredAt).toLocaleString()}</time><Icon name="chevron" size={16}/></Link>)}</div> : <div className="phase6-empty"><Icon name="sparkles"/><strong>No activity yet</strong><p>Finish a study timer or update chapter progress and it will appear here automatically.</p></div>}</CardBody></Card></div>;
}
