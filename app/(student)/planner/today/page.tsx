...
export default async function TodayPlanPage() {
  const [model, pendingReflection] = await Promise.all([getTodayPlanDisplayModel(), getPendingStudyReflectionPrompt()]);
  ...
  return <div className="phase9-page today-plan-page">
    ...
    <section className="phase9-metrics" aria-label="Today overview"><Card><CardBody><Icon name="calendar"/><div><span>Days remaining</span><strong>{model.daysRemaining ?? "—"}</strong><small>{model.forecast.attemptAnchorDate ? "to selected attempt" : "verified date unavailable"}</small></div></CardBody></Card><Card><CardBody><Icon name="target"/><div><span>Planned study</span><strong>{model.plannedMinutes}m</strong><small>in today’s current list</small></div></CardBody></Card><Card><CardBody><Icon name="clock"/><div><span>Completed today</span><strong>{model.completedStudyMinutes}m</strong><small>from finished study sessions</small></div></CardBody></Card></section>
