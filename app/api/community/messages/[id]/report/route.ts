...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const reportId = await reportCommunityMessage(id, body.reason, typeof body.details === "string" ? body.details : null);
    return NextResponse.json({ ok: true, reportId }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
