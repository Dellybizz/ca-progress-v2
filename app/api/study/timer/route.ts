...
export async function POST(request: Request) {
  ...
  if (body.action === "start") {
    ...
    result = await supabase.rpc("study_timer_start", { p_subject_id: body.subjectId || null, p_chapter_id: body.chapterId || null, p_mode: body.mode ?? "stopwatch", p_focus_target_seconds: focus, p_break_target_seconds: rest, p_timezone: body.timezone || "UTC" });
  } else if (body.action === "pause") result = await supabase.rpc("study_timer_pause", {});
  else if (body.action === "resume") result = await supabase.rpc("study_timer_resume", {});
  else if (body.action === "finish") result = await supabase.rpc("study_timer_finish", {});
  else if (body.action === "discard") result = await supabase.rpc("study_timer_discard", {});
  else if (body.action === "touch") result = await supabase.rpc("study_timer_touch", {});
