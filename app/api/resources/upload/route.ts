...
export async function POST(request: Request) {
  try {
    ...
    catch (error) { return jsonError(error instanceof Error ? error.message : "Upload validation failed.", 400, "UPLOAD_VALIDATION_FAILED"); }
    ...
    if (access.limitBytes !== null && access.usedBytes + validated.sizeBytes > access.limitBytes) {
      return jsonError(access.upgradeMessage || "Your plan storage allowance has been reached.", 403, "STORAGE_LIMIT_REACHED", { feature: "resources.storage", plan: access.tier, usedBytes: access.usedBytes, limitBytes: access.limitBytes, requestedBytes: validated.sizeBytes });
    }
    ...
    const inserted = await admin.from("uploaded_resources").insert({ owner_user_id: identity.id, title, description, subject_id: subjectId, chapter_id: chapterId, original_filename: file.name, safe_filename: validated.safeFilename, storage_bucket: RESOURCE_R2_STORAGE_BUCKET, storage_path: storagePath, mime_type: validated.mimeType, extension: validated.extension, size_bytes: validated.sizeBytes, visibility }).select("id,moderation_status").single();
    if (inserted.error) { await bucket.delete(storagePath).catch(() => undefined); return jsonError(inserted.error.message, 400, "RESOURCE_METADATA_FAILED"); }
    return NextResponse.json({ id: inserted.data.id, status: inserted.data.moderation_status, storage: RESOURCE_R2_STORAGE_BUCKET, allowance: { usedBytes: access.usedBytes + validated.sizeBytes, limitBytes: access.limitBytes, plan: access.tier } }, { status: 201, headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
