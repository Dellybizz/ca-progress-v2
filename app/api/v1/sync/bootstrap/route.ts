...
export async function GET(request: NextRequest) {
  ...
  const settled=await Promise.allSettled([
    ...
    db.prepare(`SELECT 'profile' AS entity_type,user_id AS entity_id,0 AS entity_version,
      json_object('displayName',display_name,'level',ca_level,'attempt',attempt_key,'group',group_choice,'timezone',timezone,'dailyTargetMinutes',daily_target_minutes) AS payload_json,
    ...
    context.selection?db.prepare(`SELECT DISTINCT 'attempt' AS entity_type,a.attempt_key AS entity_id,0 AS entity_version,
      json_object('attemptKey',a.attempt_key,'label',a.label,'startDate',a.start_date,'endDate',a.end_date,'status',a.status,'verificationStatus',a.verification_status) AS payload_json,NULL AS deleted_at,a.updated_at
      FROM exam_attempts a JOIN attempt_syllabus_map m ON m.attempt_key=a.attempt_key AND m.level_id=a.level_id
      JOIN course_levels l ON l.id=m.level_id JOIN course_groups g ON g.id=m.group_id
      WHERE a.verification_status='verified' AND l.code=?1 AND (?2='both' OR g.code=?2)
      ORDER BY a.start_date LIMIT 30`).bind(context.selection.level,context.selection.group).all():Promise.resolve({results:[]}),
