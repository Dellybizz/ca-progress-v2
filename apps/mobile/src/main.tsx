...
function AppShell() {
  ...
  const content = route === "settings" ? <Settings authenticated={authenticated} repository={repository} recovered={Boolean(repository?.database.recovered)} onLogout={async()=>{await repository?.lock();await logoutNative();reset();}} onRemove={async()=>{await repository?.removeFromDevice();reset();}} onWipe={async()=>{await wipeAllOfflineData();reset();}} /> : route === "today" ? <Today workspace={workspace} repository={repository}/> : route === "progress" ? <Progress workspace={workspace} repository={repository}/> : route === "syllabus"?<Syllabus workspace={workspace}/>:route === "focus" ? <Focus repository={repository}/> : route === "community" ? <Community dashboard={dashboard} repository={repository} authenticated={authenticated} account={account}/> : route==="notes"?<Notes workspace={workspace} repository={repository}/>:route==="activity"?<Activity workspace={workspace}/>:route==="buddy"?<Buddy workspace={workspace}/>:route==="profile"?<Profile workspace={workspace}/>:<Planner workspace={workspace} repository={repository}/>;
