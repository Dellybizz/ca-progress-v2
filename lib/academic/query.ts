...
import {
  contextAllowsSubject,
  type StudentContextContract,
} from "./student-context";
import { visibleAttemptKeys } from "@/lib/profile/validation";
...
type AttemptMapRow =
  Database["public"]["Tables"]["attempt_syllabus_map"]["Row"];
...
function levelDto(row: LevelRow): AcademicLevel {
  return {
    id: row.id,
    code: row.code as AcademicLevelCode,
    name: row.name,
...
async function loadRawAcademic(): Promise<RawAcademic> {
  ...
  const [levels, groups, subjects, versions, chapters, topics, attemptMap] =
    await Promise.all([
      client
        .from("course_levels")
      ...
      client
        .from("course_groups")
        .select(
          "id,level_id,code,name,sort_order,is_default,is_active,created_at,updated_at",
        )
        .eq("is_active", true)
        .order("sort_order"),
      client
        .from("subjects")
        .select(
          "id,code,slug,paper_label,title,subject_kind,level_id,group_id,source_url,is_active,sort_order,created_at,updated_at",
        )
        .eq("is_active", true)
        .order("sort_order"),
      client
        ...
        .select(
          "id,version_key,title,status,effective_from,effective_to,supersedes_version_id,source_url,source_label,source_verified_at,subject_id,content_hash,created_at,updated_at,verification_method",
        )
        .order("effective_from", { ascending: false }),
      client
        .from("chapters")
        .select(
          "id,stable_key,chapter_number,title,section_key,chapter_kind,syllabus_version_id,sort_order,slug,source_url,created_at,updated_at",
        )
        .order("sort_order"),
      client
        .from("topics")
        .select(
          "id,stable_key,unit_number,title,topic_kind,chapter_id,sort_order,source_url,created_at,updated_at",
        )
        .order("sort_order"),
      client
        .from("attempt_syllabus_map")
        .select(
          "id,level_id,attempt_key,group_id,subject_id,syllabus_version_id,created_at",
        )
        .order("attempt_key"),
    ]);
  const firstError = [
...
async function getCachedRawAcademic() {
  return getSharedPublicJson({
    ...
    key: "catalog-v1",
    ttlSeconds: 3600,
    load: loadRawAcademic,
  });
}

function versionForSubject(
  raw: RawAcademic,
  subjectId: string,
  attempt?: string | null,
) {
  if (attempt) {
    const mapped = raw.attemptMap.find(
      (item) => item.attempt_key === attempt && item.subject_id === subjectId,
    );
    return mapped
      ? (raw.versions.find((item) => item.id === mapped.syllabus_version_id) ??
          null)
      : null;
  }
  return (
    raw.versions
      .filter(
        (item) => item.subject_id === subjectId && item.status === "published",
      )
...
function subjectDto(
  ...
  const chapters: AcademicChapter[] = raw.chapters
    .filter((item) => item.syllabus_version_id === version.id)
    .map((chapter) => ({
      id: chapter.id,
...
function normalizeSelection(raw: RawAcademic, selection: AcademicSelection) {
  ...
  if (!levels.length)
    throw new AcademicDataError("No academic levels are published.");
  const selectedLevel =
    levels.find((level) => level.code === selection.level) ?? levels[0];
  const groupsForLevel = raw.groups.filter(
    (group) => group.level_id === selectedLevel.id,
  );
  const isAggregateGroup =
    selection.group === "all" || selection.group === "both";
  const requestedGroup = isAggregateGroup
    ? "all"
    : selection.group &&
        groupsForLevel.some((group) => group.code === selection.group)
      ? selection.group
      : null;
  const selectedGroup =
    requestedGroup ??
    groupsForLevel.find((group) => group.is_default)?.code ??
    groupsForLevel[0]?.code ??
    "all";
  const attempts = visibleAttemptKeys(
    [
      ...new Set(
        raw.attemptMap
          .filter((item) => item.level_id === selectedLevel.id)
  ...
  const selectedAttempt =
    selection.attempt && attempts.includes(selection.attempt)
      ? selection.attempt
      : null;
  return {
    levels,
...
function allowedGroupIds(groups: GroupRow[], selectedGroup: string) {
  return new Set(
    (selectedGroup === "all" || selectedGroup === "both"
      ? groups
      : groups.filter((group) => group.code === selectedGroup)
    ).map((group) => group.id),
  );
}
...
export async function getAcademicCatalog(
  ...
  const subjects = raw.subjects
    .filter(
      (subject) =>
        subject.level_id === normalized.selectedLevel.id &&
        groupIds.has(subject.group_id),
    )
    .map((subject) => {
      const version = versionForSubject(
        raw,
        subject.id,
        normalized.selectedAttempt,
      );
  ...
  return {
    ...
    sourceVerifiedAt:
      subjects
        .map((subject) => subject.version.sourceVerifiedAt)
        .sort()
        .at(-1) ?? null,
  };
}
...
export async function getSubjectBySlugForContext(
  slug: string,
  context: StudentContextContract,
): Promise<AcademicSubject | null> {
...
export async function searchAcademicCatalog(
  query: string,
  selection: AcademicSelection = {},
  limit = 24,
  ...
  for (const topic of raw.topics)
    topicsByChapter.set(topic.chapter_id, [
      ...(topicsByChapter.get(topic.chapter_id) ?? []),
      topic,
    ]);
  ...
  for (const subject of raw.subjects) {
    if (
      subject.level_id !== normalized.selectedLevel.id ||
      !allowedGroups.has(subject.group_id)
    )
      continue;
    const version = versionForSubject(
      raw,
    ...
    const base = {
      ...
      levelCode: level.code as AcademicLevelCode,
      groupCode: group.code,
    };
    if (
      `${subject.paper_label} ${subject.title} ${subject.code}`
        .toLocaleLowerCase()
        .includes(q)
    )
      results.push({
        ...
        id: subject.id,
        title: subject.title,
        subtitle: `${level.name} · ${group.name} · ${subject.paper_label}`,
        ...base,
      });
    for (const chapter of raw.chapters.filter(
      (item) => item.syllabus_version_id === version.id,
    )) {
      if (
        `${chapter.chapter_number} ${chapter.title} ${chapter.section_key ?? ""}`
          .toLocaleLowerCase()
          .includes(q)
      )
        results.push({
          ...
          id: chapter.id,
          title: chapter.title,
          subtitle: `${subject.title} · Chapter ${chapter.chapter_number}`,
          chapterId: chapter.id,
          ...base,
        });
      for (const topic of topicsByChapter.get(chapter.id) ?? []) {
        if (
          `${topic.unit_number ?? ""} ${topic.title}`
            .toLocaleLowerCase()
            .includes(q)
        )
          results.push({
            ...
            id: topic.id,
            title: topic.title,
            subtitle: `${subject.title} · ${chapter.title}`,
            chapterId: chapter.id,
            ...base,
          });
      }
...
export async function getAcademicVersionPreview(): Promise<
  AcademicVersionPreview[]
> {
  const raw = await getCachedRawAcademic();
  const subjectById = new Map(
    raw.subjects.map((subject) => [subject.id, subject]),
  );
  const levelById = new Map(raw.levels.map((level) => [level.id, level]));
  const groupById = new Map(raw.groups.map((group) => [group.id, group]));
  const previews = raw.versions.map((version) => {
    ...
    if (!subject)
      throw new AcademicDataError("A syllabus version is missing its subject.");
    const level = levelById.get(subject.level_id);
    const group = groupById.get(subject.group_id);
    if (!level || !group)
      throw new AcademicDataError("A subject is missing its level or group.");
    const chapterIds = raw.chapters
      .filter((chapter) => chapter.syllabus_version_id === version.id)
    ...
    return {
      ...
      topicCount: raw.topics.filter((topic) => chapterSet.has(topic.chapter_id))
        .length,
    };
  });
