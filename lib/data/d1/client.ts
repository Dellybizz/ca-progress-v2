  ...
  private async execute():Promise<QueryResult>{const startedAt=performance.now();let rowCount:number|null=null;try{
    ...
    if(this.operation==="insert"||this.operation==="upsert"){
      ...
      for(const input of inputs){const row=await this.preparedRow(input);const keys=Object.keys(row);if(!keys.length)throw new Error("Insert payload is empty.");const values=keys.map(k=>encodeValue(row[k]));let sql=`INSERT INTO ${ident(this.table)}(${keys.map(ident).join(",")}) VALUES(${keys.map(()=>"?").join(",")})`;
        if(this.operation==="upsert"){if(!this.conflict.length)throw new Error("D1 upsert requires onConflict.");const info=await tableInfo(this.db,this.table);const primaryKeys=new Set(info.filter(column=>column.pk>0).map(column=>column.name));const updates=keys.filter(k=>!this.conflict.includes(k)&&!primaryKeys.has(k));sql+=` ON CONFLICT(${this.conflict.map(ident).join(",")}) ${updates.length?`DO UPDATE SET ${updates.map(k=>`${ident(k)}=excluded.${ident(k)}`).join(",")}`:"DO NOTHING"}`;}
