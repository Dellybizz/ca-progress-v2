...
type DoubtRow = { id: string; resolved_at: string };
type GoalRow = { id: string; goal_kind: "daily_study" | "weekly_study"; completed_at: string };
type TogetherRow = { id: string; completed_at: string };
...
async function loadCanonicalSources(userId: string) {
  ...
  const [sessions, today, progress, tests, reflections, doubts, goals, together, profile] = await Promise.all([
    ...
    db.prepare(`SELECT session_id,reflection_saved_at FROM study_session_phase3
      WHERE user_id=?1 AND reflection_saved_at IS NOT NULL
      ORDER BY reflection_saved_at ASC,session_id ASC LIMIT 5000`).bind(userId).all<ReflectionRow>(),
    db.prepare(`SELECT id,resolved_at FROM study_session_doubts
      WHERE user_id=?1 AND status='resolved' AND resolved_at IS NOT NULL
      ORDER BY resolved_at ASC,id ASC LIMIT 5000`).bind(userId).all<DoubtRow>(),
    db.prepare(`SELECT g.id,e.goal_kind,g.completed_at FROM goals g
      ...
      WHERE g.user_id=?1 AND g.completed_at IS NOT NULL AND e.goal_kind IN ('daily_study','weekly_study')
      ORDER BY g.completed_at ASC,g.id ASC LIMIT 5000`).bind(userId).all<GoalRow>(),
    db.prepare(`SELECT s.id,COALESCE(s.ended_at,MAX(p.completed_at)) AS completed_at
      ...
      WHERE s.status='completed' AND mine.canonical_study_session_id IS NOT NULL AND mine.completed_at IS NOT NULL
      GROUP BY s.id,s.ended_at
      HAVING COUNT(*)=2
        AND SUM(CASE WHEN p.canonical_study_session_id IS NOT NULL AND p.completed_at IS NOT NULL THEN 1 ELSE 0 END)=2
      ORDER BY completed_at ASC,s.id ASC LIMIT 2000`).bind(userId).all<TogetherRow>(),
...
function candidatesFromSources(sources: Awaited<ReturnType<typeof loadCanonicalSources>>) {
  ...
  const todayDays = sources.today.map((row) => ({ row, localDate: /^\d{4}-\d{2}-\d{2}/.test(row.scheduled_for) ? row.scheduled_for.slice(0, 10) : localDateKey(row.completed_at, row.timezone) }));
  ...
  for (const row of sources.reflections) candidates.push({ eventKey: xpEventKey("reflection", row.session_id), eventType: "reflection", sourceType: "study_session_reflection", sourceId: row.session_id, xp: xpForEvent("reflection"), occurredAt: row.reflection_saved_at });
  for (const row of sources.doubts) candidates.push({ eventKey: xpEventKey("resolved_doubt", row.id), eventType: "resolved_doubt", sourceType: "study_session_doubt", sourceId: row.id, xp: xpForEvent("resolved_doubt"), occurredAt: row.resolved_at });
  for (const row of sources.goals) {
    const eventType = row.goal_kind === "weekly_study" ? "weekly_goal" : "daily_goal";
    candidates.push({ eventKey: xpEventKey(eventType, row.id), eventType, sourceType: "planner_goal", sourceId: row.id, xp: xpForEvent(eventType), occurredAt: row.completed_at, metadata: { goalKind: row.goal_kind } });
  }
  for (const row of sources.together) candidates.push({ eventKey: xpEventKey("study_together", row.id), eventType: "study_together", sourceType: "study_together_session", sourceId: row.id, xp: xpForEvent("study_together"), occurredAt: row.completed_at });
