...
type BillingPath = "/create-order" | "/verify" | "/webhook" | "/entitlement" | "/health" | "/create-subscription" | "/verify-subscription" | "/subscription-action";
type BillingInvoke = { path: BillingPath; method?: "GET" | "POST"; userId?: string | null; query?: string; body?: string; contentType?: string; razorpaySignature?: string | null; razorpayEventId?: string | null };
...
export async function invokeBillingService(input: BillingInvoke) {
  ...
  const mutatesAccess = input.path === "/verify" || input.path === "/webhook" || input.path === "/verify-subscription" || input.path === "/subscription-action";
