...
export type SubscriptionPlan = {
  id: string; tier_key: PlanTier; billing_cycle: BillingCycle; name: string; tagline: string; rank: number;
  price_subunits: number | null; currency: string; duration_value: number; duration_unit: string;
  active: boolean; checkout_enabled: boolean; sort_order: number;
};
...
export type BillingModel = {
  ...
  currentSubscription?: { id: string; status: string; starts_at: string; ends_at: string | null; source: string } | null;
  payments?: Array<{ id: string; plan_id: string; provider_order_id: string; provider_payment_id: string | null; amount_subunits: number; currency: string; status: string; created_at: string; paid_at: string | null }>;
  events?: Array<{ id: string; plan_id: string; event_type: string; source: string; starts_at: string | null; ends_at: string | null; created_at: string }>;
  plans?: SubscriptionPlan[];
};

function config() {
  const value = getSupabaseAdminRuntimeConfig();
  if (!value.configured) throw new Error("V2 billing database configuration is unavailable.");
  return value;
}
...
async function rest(path: string, init: RequestInit = {}) {
  ...
  if (!response.ok) {
    const message = data && typeof data === "object" && "message" in data ? String((data as { message?: unknown }).message) : `Billing data request failed (${response.status}).`;
...
export async function listPlans(): Promise<SubscriptionPlan[]> {
  const data = await rest("subscription_plans?active=eq.true&select=id,tier_key,billing_cycle,name,tagline,rank,price_subunits,currency,duration_value,duration_unit,active,checkout_enabled,sort_order&order=sort_order.asc");
  return Array.isArray(data) ? data as SubscriptionPlan[] : [];
}

export async function listPlanEntitlements(): Promise<PlanEntitlement[]> {
  const data = await rest("plan_entitlements?select=plan_id,feature_key,enabled,limit_value,limit_unit,reset_period,upgrade_message");
  return Array.isArray(data) ? data as PlanEntitlement[] : [];
}
...
export async function getEntitlementForUser(userId: string, featureKey: string): Promise<Entitlement> {
  ...
  return {
    planId: String(row.plan_id ?? ""), tier: String(row.tier_key ?? "free") as PlanTier, planName: String(row.plan_name ?? "Free"),
    featureKey: String(row.feature_key ?? featureKey), allowed: Boolean(row.allowed), limitValue: row.limit_value === null || row.limit_value === undefined ? null : Number(row.limit_value),
    limitUnit: String(row.limit_unit ?? "unlimited"), resetPeriod: String(row.reset_period ?? "never"), upgradeMessage: String(row.upgrade_message ?? "Upgrade your plan to use this feature."),
  };
}
...
export async function getPricingModel() {
  ...
  if (identity) {
    const data = await rest("rpc/phase11_current_plan_id", { method: "POST", body: JSON.stringify({ p_user_id: identity.id }) });
    currentPlanId = typeof data === "string" ? data : null;
  }
...
export async function getBillingModel(): Promise<BillingModel> {
  ...
  const planIdData = await rest("rpc/phase11_current_plan_id", { method: "POST", body: JSON.stringify({ p_user_id: identity.id }) });
  const currentPlanId = typeof planIdData === "string" ? planIdData : "";
  const currentPlan = plans.find((plan) => plan.id === currentPlanId) ?? plans.find((plan) => plan.tier_key === "free" && plan.billing_cycle === "free")!;
  const subscriptionsData = await rest(`user_subscriptions?user_id=eq.${encodeURIComponent(identity.id)}&select=id,plan_id,status,starts_at,ends_at,source,created_at&order=created_at.desc&limit=25`);
  const subscriptions = Array.isArray(subscriptionsData) ? subscriptionsData as Array<{ id: string; plan_id: string; status: string; starts_at: string; ends_at: string | null; source: string; created_at: string }> : [];
