...
export async function optionalUser(): Promise<ServerIdentity | null> { if (!getSupabasePublicConfig().configured) return null; const supabase = await createServerSupabaseClient(); const { data, error } = await supabase.auth.getClaims(); const claims = data?.claims as Record<string, unknown> | undefined; if (error || !claims) return null; const id = claimString(claims, "sub"); if (!id) return null; return { id, email: claimString(claims, "email") }; }
