...
export type ServerIdentity = { id: string; email: string | null; phone: null };
...
export async function optionalUser(): Promise<ServerIdentity | null> { if (!getSupabasePublicConfig().configured) return null; const supabase = await createServerSupabaseClient(); const { data, error } = await supabase.auth.getClaims(); const claims = data?.claims as Record<string, unknown> | undefined; if (error || !claims) return null; const id = claimString(claims, "sub"); if (!id) return null; return { id, email: claimString(claims, "email"), phone: null }; }
...
export async function resolvePostAuthDestination(next: string) {
  ...
  if (!profile.feature_guide_completed_at) return `/onboarding/guide?next=${encodeURIComponent(safeNext)}`;
  return safeNext;
}
export async function loadViewer(): Promise<Viewer> { const user = await optionalUser(); if (!user) return { authenticated: false, label: "Guest", initial: "G" }; const profile = await getProfileForUser(user.id); const label = profile?.display_name || user.email || "Student"; return { authenticated: true, id: user.id, label, initial: label.trim().charAt(0).toUpperCase() || "S" }; }
