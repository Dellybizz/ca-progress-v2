...
import type {
  DashboardAcademicReference,
  DashboardAcademicSubject,
  DashboardIcaiUpdate,
  DashboardLiveReference,
} from "./types";
...
async function loadAcademicReference(levelCode: string, groupChoice: string, attemptKey: string): Promise<DashboardAcademicReference | null> {
  ...
  const allowedGroups = new Set(
    (aggregateGroups ? groups : groups.filter((group) => group.code === groupChoice)).map((group) => group.id),
  );
  ...
  const academicSubjects: DashboardAcademicSubject[] = subjects
    .filter((subject) => allowedGroups.has(subject.group_id) && subjectIds.has(subject.id))
    .map((subject) => {
      const versionId = versionBySubject.get(subject.id);
      ...
      return {
        ...
        chapterCount: versionId ? chapters.filter((chapter) => chapter.syllabus_version_id === versionId).length : 0,
      };
    });

  return {
    level: { id: level.id, code: level.code, name: level.name },
    groups: groups.filter((group) => aggregateGroups || allowedGroups.has(group.id)).map((group) => ({ id: group.id, code: group.code, name: group.name })),
    subjects: academicSubjects,
    totalChapters: academicSubjects.reduce((sum, subject) => sum + subject.chapterCount, 0),
  };
}

const cachedAcademicReference = unstable_cache(
  loadAcademicReference,
  ["phase4-dashboard-academic-v1"],
  { revalidate: 900 },
);
...
async function loadLiveReference(
  ...
  today: string,
): Promise<DashboardLiveReference> {
  const supabase = createReferenceClient();
  ...
  const [eventResponse, resourceResponse, sourceResponse, attemptMapResponse, subjectMapResponse] = await Promise.all([
    attempt
      ? supabase.from("exam_events").select("*").eq("attempt_id", attempt.id).eq("verification_status", "verified").gte("event_date", today).order("event_date").limit(24)
  ...
  for (const row of (attemptMapResponse.data ?? []) as ResourceAttemptRow[]) {
  ...
  for (const row of (subjectMapResponse.data ?? []) as ResourceSubjectRow[]) {
  ...
  const updates: DashboardIcaiUpdate[] = ((resourceResponse.data ?? []) as ResourceRow[])
    .filter((resource) => {
      const mappedAttempts = attemptIdsByResource.get(resource.id) ?? [];
      const mappedSubjects = subjectIdsByResource.get(resource.id) ?? [];
      const metadata = jsonRecord(resource.metadata);
      const metadataLevels = Array.isArray(metadata.level_codes)
        ? metadata.level_codes.filter((value): value is string => typeof value === "string")
        : [];
      const attemptMatch = Boolean(attempt && mappedAttempts.includes(attempt.id));
      const subjectMatch = mappedSubjects.some((id) => selectedSubjectIds.has(id));
      if (mappedAttempts.length && !attemptMatch) return false;
      if (mappedSubjects.length && !subjectMatch) return false;
      return metadataLevels.includes(levelCode) || attemptMatch || subjectMatch;
    })
    .slice(0, 4)
    .map((resource) => {
  ...
  const examEvents = (eventResponse.data ?? [])
    .filter((event) => event.event_type === "exam_start" || event.event_type === "exam_paper")
    .map((event) => ({
      id: event.id,
      title: event.title,
  ...
  const verifiedCandidates = [
    attempt?.last_seen_at ?? null,
    ...examEvents.map((event) => event.lastVerifiedAt),
    ...updates.map((update) => update.lastVerifiedAt),
  ...
  return {
    attempt: attempt ? {
      ...
      lastVerifiedAt: attempt.last_seen_at,
    } : null,
    examEvents,
    updates,
    verifiedAt: verifiedCandidates.sort().at(-1) ?? null,
  };
}

const cachedLiveReference = unstable_cache(
  loadLiveReference,
  ["phase4-dashboard-live-v1"],
  { revalidate: 60 },
);

export function getDashboardAcademicReference(levelCode: string, groupChoice: string, attemptKey: string) {
  return cachedAcademicReference(levelCode, groupChoice, attemptKey);
}

export function getDashboardLiveReference(input: { levelId: string; levelCode: string; attemptKey: string; subjectIds: string[]; today: string }) {
  return cachedLiveReference(input.levelId, input.levelCode, input.attemptKey, input.subjectIds.slice().sort().join(","), input.today);
}
