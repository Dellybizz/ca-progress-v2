...
async function loadLiveReference(levelId: string, levelCode: string, attemptKey: string, subjectIdsKey: string, today: string): Promise<DashboardLiveReference> {
  ...
  const examEvents = ((eventResponse.data ?? []) as EventRow[])
    .filter((event) => (event.event_type === "exam_start" || event.event_type === "exam_paper") && (event.subject_id ? selectedSubjectIds.has(event.subject_id) : event.event_type === "exam_start"))
    .map((event) => ({ id:event.id,title:event.title,eventType:event.event_type,eventDate:event.event_date,subjectId:event.subject_id,sourceUrl:event.source_url,lastVerifiedAt:event.last_seen_at }));
