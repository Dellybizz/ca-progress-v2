...
export type DashboardIcaiUpdate = { id: string; type: string; title: string; summary: string | null; officialUrl: string; sourceName: string; sourceUrl: string; publishedOn: string | null; lastVerifiedAt: string; lastChangedAt: string };
export type DashboardLiveReference = { attempt: DashboardAttemptReference | null; examEvents: DashboardExamEvent[]; updates: DashboardIcaiUpdate[]; verifiedAt: string | null };
export type DashboardReadyModel = {
  ...
  countdown: { status: "scheduled" | "past" | "awaiting_verified_date"; daysRemaining: number | null; targetDate: string | null; title: string; sourceUrl: string | null; lastVerifiedAt: string | null; sourceKind: "exam_event" | "attempt" | "none" };
  ...
  icai: { updates: DashboardIcaiUpdate[]; verifiedAt: string | null };
  alerts: Array<{ kind: "revision" | "test" | "streak"; title: string; description: string; phase: 5 | 6 | 9 }>;
  recommendation: { slot: "next_study"; status: "contextual_fallback" | "empty"; title: string; description: string; href: string; phase9Ready: true };
  quickActions: DashboardQuickAction[];
};
export type DashboardPageModel =
  | { mode: "guest"; generatedAt: string; viewer: { authenticated: false; displayName: "Guest" } }
  | { mode: "onboarding"; generatedAt: string; viewer: { authenticated: true; id: string; displayName: string }; reason: "profile_incomplete" }
  | DashboardReadyModel;
