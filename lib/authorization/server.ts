...
export async function getAdminOperator(): Promise<{ allowed: boolean; user: ServerIdentity | null; role: AppRole }> { const user = await optionalUser(); if (!user) return { allowed: false, user: null, role: "student" }; const role = await getServerAppRole(); return { allowed: canEnterAdminArea(role), user, role }; }
