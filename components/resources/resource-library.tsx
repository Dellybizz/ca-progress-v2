...
export function ResourceLibrary({ model, initialTab = "my", initialAcademicContext }: { model: ResourceLibraryReady; initialTab?: "my" | "shared" | "icai"; initialAcademicContext?: AcademicContext }) {
  const initialContext = resolveAcademicContext(model, initialAcademicContext);
  const [tab, setTab] = useState(initialTab); const [query, setQuery] = useState(""); const [newNote, setNewNote] = useState(false); const [upload, setUpload] = useState(false); const [academicFilter, setAcademicFilter] = useState(initialContext);
  ...
  const myNotes = useMemo(() => model.myNotes.filter((note) => (!academicFilter.subjectId || note.subjectId === academicFilter.subjectId) && (!academicFilter.chapterId || note.chapterId === academicFilter.chapterId) && (!q || matches(`${note.title} ${note.excerpt} ${note.subjectTitle ?? ""} ${note.chapterTitle ?? ""} ${note.tags.join(" ")}`, q))), [academicFilter.chapterId, academicFilter.subjectId, model.myNotes, q]);
