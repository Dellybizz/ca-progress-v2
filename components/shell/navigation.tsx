...
const studentGroups: StudentNavGroup[] = [
  { key: "study", label: "Study", icon: "timer", items: [
    { label: "Today Plan", href: "/planner/today", icon: "sparkles" },
    { label: "Study", href: "/study", icon: "timer" },
    { label: "Planner", href: "/planner", icon: "calendar", exact: true },
    { label: "Calendar", href: "/calendar", icon: "calendar" },
  ] },
  { key: "progress", label: "Progress", icon: "chart", items: [
  ...
  { key: "resources", label: "Resources", icon: "book", items: [
    { label: "Syllabus", href: "/syllabus", icon: "book" },
    { label: "ICAI Updates", href: "/updates", icon: "bell" },
    { label: "Resources", href: "/resources", icon: "book" },
    { label: "ICAI Resources", href: "/resources/icai", icon: "shield" },
...
function StudentDesktopNavigation({ pathname }: { pathname: string }) {
  const activeGroup = useMemo(() => studentGroups.find((group) => group.items.some((item) => itemIsActive(item, pathname)))?.key ?? null, [pathname]);
  ...
  return <nav className="sidebar-nav sidebar-nav--grouped" aria-label="student navigation">
    ...
    <div className="sidebar-nav-groups">
      {studentGroups.map((group) => {
        ...
        return <div className={`sidebar-nav-group${expanded ? " is-open" : ""}${hasActiveItem ? " has-active-item" : ""}`} key={group.key}>
          <button type="button" className="sidebar-nav-group__trigger" aria-expanded={expanded} onClick={() => setManualGroup({ pathname, key: expanded ? null : group.key })}>
            <Icon name={group.icon} size={18}/><span>{group.label}</span><span className="sidebar-nav-group__chevron" aria-hidden="true"><Icon name="chevron" size={13}/></span>
          </button>
          {expanded ? <div className="sidebar-nav-group__items">{group.items.map((item) => <NavLink key={item.href} item={item} pathname={pathname} compact/>)}</div> : null}
        </div>;
      })}
    </div>
  </nav>;
