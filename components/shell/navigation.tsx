...
function NavLink({ item, pathname, compact = false }: { item: NavItem; pathname: string; compact?: boolean }) {
  ...
  return <Link prefetch={true} href={item.href} className={`${active ? "is-active" : ""}${compact ? " sidebar-nav__child" : ""}`} aria-current={active ? "page" : undefined}>
    <Icon name={item.icon} size={compact ? 15 : 17}/><span>{item.label}</span>{active ? <i className="sidebar-nav__active" aria-hidden="true"/> : null}
  </Link>;
}
      ...
      {studentGroups.map((group) => {
        ...
        return <div className={`sidebar-nav-group${expanded ? " is-open" : ""}${hasActiveItem ? " has-active-item" : ""}`} key={group.key}>
          <button type="button" className="sidebar-nav-group__trigger" aria-expanded={expanded} onClick={() => setManualGroup({ pathname, key: expanded ? null : group.key })}>
            <Icon name={group.icon} size={16}/><span>{group.label}</span><span className="sidebar-nav-group__chevron" aria-hidden="true"><Icon name="chevron" size={12}/></span>
          </button>
