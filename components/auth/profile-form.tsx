...
export function ProfileForm({
  ...
  const [level, setLevel] = useState<CALevel | "">(
    (profile.ca_level as CALevel | null) ?? "",
  );
  const [group, setGroup] = useState<GroupChoice | "">(
    (profile.group_choice as GroupChoice | null) ?? "",
  );
  const [attemptKey, setAttemptKey] = useState(profile.attempt_key ?? "");
  const [target, setTarget] = useState(
    String(profile.daily_target_minutes ?? 120),
  );
  ...
  const applicableAttempts = useMemo(
    () =>
      prerequisitesReady && level
        ? attempts.filter((option) =>
            attemptAppliesToSelection(option, level, effectiveGroup),
          )
        : [],
    [attempts, effectiveGroup, level, prerequisitesReady],
  );
  ...
  async function saveProfile() {
    ...
    if (!response.ok || !result.ok)
      return setStatus({ error: result.error || "Could not save profile." });
  ...
  async function uploadAvatar(file: File | undefined) {
    ...
    setUploading(false);
    if (!response.ok || !result.ok)
      return setStatus({ error: result.error || "Could not upload avatar." });
    setAvatarUrl(result.signedUrl ?? null);
    setStatus({ success: "Avatar updated." });
    router.refresh();
  }
  ...
  return (
    <div className="profile-v2">
      <div className="profile-v2__hero">
        <div
        ...
        <div>
          <Badge tone="brand">Private profile</Badge>
          <h1>Your CA Progress profile</h1>
          <p>
            These settings are scoped to your authenticated user through
            database RLS.
          </p>
        </div>
      </div>
      ...
      <div className="profile-v2__grid">
        <Card>
          <CardHeader title="Identity" description={identityLabel} />
          <CardBody>
            <div className="profile-form-stack">
              ...
              <label className="avatar-upload">
                ...
                <span className="avatar-upload__control">
                  <input
                    type="file"
                ...
                <small>
                  JPEG, PNG or WebP · maximum 2 MB · stored privately.
                </small>
              </label>
            </div>
          </CardBody>
        ...
        <Card>
          <CardHeader
            title="Study profile"
            description="Canonical academic selection"
          />
          <CardBody>
            <div className="profile-form-stack">
              <Select
                ...
                value={level}
                onChange={(event) => {
                  const nextLevel = event.target.value as CALevel;
                  setLevel(nextLevel);
                  const nextGroup =
                  ...
                  const nextAttempts = attempts.filter((option) =>
                    attemptAppliesToSelection(option, nextLevel, nextGroup),
                  );
                  if (!nextAttempts.some((option) => option.key === attemptKey))
                    setAttemptKey("");
                  if (nextLevel === "foundation") setGroup("not_applicable");
              ...
              {level !== "foundation" ? (
                <Select
                  ...
                  value={group}
                  onChange={(event) => {
                    const nextGroup = event.target.value as GroupChoice;
                    setGroup(nextGroup);
                    const nextAttempts = level
                      ? attempts.filter((option) =>
                          attemptAppliesToSelection(option, level, nextGroup),
                        )
                      : attempts;
                    if (
                      !nextAttempts.some((option) => option.key === attemptKey)
                    )
                      setAttemptKey("");
                  }}
                >
                ...
                <div className="onboarding-na">
                  <Icon name="check" />
                  <span>
                    <strong>Group: not applicable</strong>
                    <small>Foundation profile</small>
                  </span>
                </div>
              )}
              <Select
              ...
              />
              <Button size="lg" isLoading={saving} onClick={saveProfile}>
                Save profile
              </Button>
            </div>
          </CardBody>
      ...
      <form action="/auth/signout" method="post" className="profile-signout">
