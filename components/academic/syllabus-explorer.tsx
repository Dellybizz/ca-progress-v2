...
export function SyllabusExplorer({ catalog }: { catalog: AcademicCatalog }) {
  ...
  useEffect(() => {
    if (search.trim().length < 2) { setResults([]); setSearchState("idle"); return; }
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setResults(payload.results ?? []); setSearchState("idle");
      } catch (error) {
