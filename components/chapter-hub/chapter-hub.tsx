...
export function ChapterHub({ model }: { model: ChapterHubReadyModel }) {
  ...
  const [resourceTab, setResourceTab] = useState<"private" | "community">("private");
  ...
  const filteredAvailableItems = useMemo(() => {
    ...
    return availableItems.filter((item) => {
      const source = item.sourceKind.startsWith("personal_") ? "private" : item.sourceKind.startsWith("community_") ? "community" : null;
  ...
  async function mutate(body:Record<string,unknown>){
    const response=await fetch(`/api/v1/chapters/${encodeURIComponent(academic.chapterId)}/workspace`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});
  ...
  return <div className={`chapter-hub-page chapter-hub-page--${mobileView}`} data-canonical-chapter-id={academic.chapterId}>
    ...
    <div className="chapter-hub-workspace">
      <main className="chapter-hub-main">
        ...
        <Card className="chapter-hub-section chapter-hub-materials" data-mobile-view="resources"><CardBody><SectionTitle icon="book" eyebrow="Chapter material" title="Attached to this chapter"/><div className="chapter-hub-resource-tabs" role="tablist" aria-label="Resource source">{(["private","community"] as const).map((tab)=><button type="button" role="tab" aria-selected={resourceTab===tab} key={tab} onClick={()=>setResourceTab(tab)}><span>{tab.charAt(0).toUpperCase()+tab.slice(1)}</span><small>{resourceCounts[tab]}</small></button>)}</div><input className="chapter-hub-resource-search" type="search" value={resourceQuery} onChange={(event)=>setResourceQuery(event.target.value)} placeholder={`Search ${resourceTab} resources…`} aria-label="Search resources to attach"/><div className="chapter-hub-resource-results">{filteredAvailableItems.length?filteredAvailableItems.slice(0,40).map((item)=><div key={`${item.sourceKind}:${item.sourceId}`}><span><code>{resourceCode(item)}</code><strong>{item.title}</strong><small>{item.meta}</small></span><button type="button" className="ui-button ui-button--secondary" onClick={()=>void attachItem(item)}>Save to chapter</button></div>):<p>No matching {resourceTab} resources.</p>}</div><div className="chapter-hub-pinned-list">{pinnedItems.length?pinnedItems.map((item)=><div key={item.id}><Link href={item.href}><span><code>{resourceCode(item)}</code><strong>{item.title}</strong><small>{item.meta}</small></span><Icon name="arrow" size={14}/></Link><button type="button" onClick={()=>void removeItem(item)} aria-label={`Remove ${item.title} from chapter`}><Icon name="close" size={14}/></button></div>):<p>Nothing attached yet.</p>}</div></CardBody></Card>
