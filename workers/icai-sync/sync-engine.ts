...
import type { IcaiLevelCode, IcaiSourceConfig, IcaiSyncSummary, ParsedIcaiResource } from "../../lib/icai/types";
...
function adminClient(runtime: IcaiSyncRuntime): AdminClient {
  if (!runtime.supabaseUrl || !runtime.serviceRoleKey) throw new Error("V2 Supabase service configuration is missing for the ICAI sync service.");
  return createClient<Database>(runtime.supabaseUrl, runtime.serviceRoleKey, {
...
async function fetchOfficialPage(source: IcaiSourceConfig, runtime: IcaiSyncRuntime) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      const headers = new Headers({ Accept: "text/html,application/xhtml+xml", "User-Agent": runtime.userAgent });
...
export async function runIcaiSyncEngine(
  ...
  const { data: run, error: runError } = await supabase
    ...
    .insert({
      ...
      details: { engine: "phase8", execution: "internal_worker", source_ids: sources.map((source) => source.id) },
    })
    .select("id")
    .single();
  ...
  try {
    for (const [index, source] of sources.entries()) {
      try {
        await processSource(supabase, runtime, run.id, source, levelResponse.data ?? [], subjectResponse.data ?? [], attemptResponse.data ?? []);
      } catch (error) {
    ...
    await supabase
      .from("icai_sync_runs")
      .update({ status: "failed", completed_at: new Date().toISOString(), error_summary: errorMessage.slice(0, 4_000) })
      .eq("id", run.id);
    throw error;
  }
}
