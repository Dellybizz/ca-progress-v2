...
const groups = Object.freeze([
  ...
  Object.freeze({ id: "notes-and-resources", routes: ["/notes", "/notes/[id]", "/resources", "/resources/[id]", "/resources/[id]/view", "/resources/icai", "/updates"], primaryAction: "Find, open or edit trusted study material", hierarchy: ["search or editor", "provenance and academic scope", "primary open or save action", "secondary metadata"], transformations: ["keep search/editor and provenance", "move filters to a sheet", "separate list and detail", "defer full evidence"] }),
  ...
  Object.freeze({ id: "account-and-commerce", routes: ["/billing", "/feature-tour", "/pricing", "/settings", "/settings/profile"], primaryAction: "Review or change one account concern", hierarchy: ["current account state", "selected concern", "save or confirm", "supporting explanation"], transformations: ["keep current value and confirmation", "merge account summaries", "move complex edits to focused flows", "remove preview-era duplication"] }),
  Object.freeze({ id: "admin-operations", routes: ["/admin", "/admin/audit", "/admin/billing", "/admin/community/moderation", "/admin/consistency", "/admin/control", "/admin/health", "/admin/icai-sync", "/admin/icai-sync/data", "/admin/jobs", "/admin/notifications", "/admin/resources/moderation", "/admin/plans", "/admin/staff", "/admin/syllabus", "/admin/users", "/admin/users/[userId]", "/admin/users/[userId]/subscription-access"], primaryAction: "Resolve the highest-priority authorized operation", hierarchy: ["operational status", "attention item", "safe action", "receipt or audit evidence"], transformations: ["keep status and authorized action", "convert tables to labelled summaries", "move filters and detail to sheets/pages", "defer low-priority metrics"] }),
]);
